...
class AESClient:
    def __init__(self, master):
        ...
        self.aes_key = None  # Will be set when user enters password
        ...
        self.password_frame = ttk.Frame(self.main_frame, padding=(0, 10, 0, 10))
        self.password_frame.grid(row=2, column=0, sticky="ew")

        self.message_frame = ttk.Frame(self.main_frame)
        self.message_frame.grid(row=3, column=0, sticky="nsew")
        ...
        # Password setup
        ttk.Label(self.password_frame, text="AES Password:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        self.password_entry = ttk.Entry(self.password_frame, width=30, show="*")
        self.password_entry.grid(row=0, column=1, padx=5, pady=5)

        self.connect_button = ttk.Button(self.password_frame, text="Connect", command=self.connect_to_server)
    ...
    def connect_to_server(self):
        ...
        self.aes_key = derive_key_from_password(password)
        ...
        try:
            ...
            messagebox.showinfo("Info", "Connected to the server with AES encryption.")
            ...
            self.user_id_display.config(text=f"Your userID: {self.user_id} (AES Encrypted)")
    ...
    def send_message(self):
        ...
        full_message = f"{recipient_id}|AES:{encrypted_message}"
            ...
            try:
                if self.client_socket:
                    ...
                    try:
                        ...
                        # Check if it's an AES message
                        if payload.startswith("AES:"):
                            encrypted_message = payload[4:]  # Remove "AES:" prefix
                            ...
                            try:
                                ...
                                display_line = f"Received from {sender_id} (Decryption Failed): Wrong password or corrupted message"
                                self.update_chat_area(display_line)
                        else:
